...
def model_cfg(test_data_dir):
    ...
    model_cfg = {
        ...
        'tokenizer': {'library': 'huggingface', 'type': 'tiiuae/mixtral-40b', 'use_fast': True},
...
@pytest.fixture()
def mixtral_gpt_model(model_cfg, trainer_cfg, precision):
...
class TestmixtralGPTModel:
    @pytest.mark.unit
    def test_constructor(self, mixtral_gpt_model):
        assert isinstance(mixtral_gpt_model, MegatronGPTModel)

        num_weights = mixtral_gpt_model.num_weights
        assert num_weights == 16827136

    @pytest.mark.unit
    def test_tokenizer(self, mixtral_gpt_model, test_text):

        assert isinstance(mixtral_gpt_model.tokenizer, AutoTokenizer)
        assert mixtral_gpt_model.tokenizer.name == 'PreTrainedTokenizerFast'
        assert mixtral_gpt_model.tokenizer.vocab_size == 65024

        ids = [mixtral_gpt_model.tokenizer.text_to_ids(text) for text in test_text]
    ...
    @pytest.mark.unit
    def test_forward(self, mixtral_gpt_model, test_text):

        dtype = mixtral_gpt_model.torch_dtype

        mixtral_gpt_model.eval()
        ...
        masks_and_position_ids = [
            get_ltor_masks_and_position_ids(id_tensor, mixtral_gpt_model.tokenizer.eos_id, False, False, False)
        ...
        with torch.no_grad():
            for tokens, attn_mask_and_pos_ids in zip(id_tensors, masks_and_position_ids):
                ...
                with torch.autocast('cuda', dtype=dtype):
                    output_tensor = mixtral_gpt_model.forward(
                        tokens=tokens.cuda(),
                ...
                assert output_tensor.shape[2] == mixtral_gpt_model.padded_vocab_size
