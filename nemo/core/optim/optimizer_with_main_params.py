...
try:
    from megatron.core.parallel_state import get_data_parallel_group, get_data_modulo_expert_parallel_group, get_data_parallel_world_size
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            for j, param in enumerate(param_group['params']):
                ...
                if main_param is not None and self._fp32_grad_accum and param.requires_grad:
                    ...
                    grad_acc.register_hook(self._make_param_hook(param, main_param, i, grad_chunk_info, is_expert_group))
                ...
                if self._grad_allreduce_chunk_size_mb > 0:
                    ...
                    while True:
                        ...
                        if allreduce_tensor is None:
                            break
                        allreduce_grads(allreduce_tensor, data_group, self._grad_div_ar_fusion, data_group_size, grad_divisor)
                else:
                    allreduce_grads(main_param.grad, data_group, self._grad_div_ar_fusion, data_group_size, grad_divisor)
        return param_hook
